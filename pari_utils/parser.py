...
#                 (C) 2021 Vincent Delecroix <vincent.delecroix@u-bordeaux.fr>
#
...
def read_pari_desc():
    ...
    >>> from pari_utils.parser import read_pari_desc
    >>> D = read_pari_desc()
    >>> Dcos = D["cos"]
...
def parse_prototype(proto, help, initial_args=[]):
    ...
    :param proto: a PARI prototype like ``"GD0,L,DGDGDG"``
    :param help: a PARI help string like ``"qfbred(x,{flag=0},{d},{isd},{sd})"``
    :param initial_args: other arguments to this function which come
      before the PARI arguments, for example a ``self`` argument.
    :returns: a tuple ``(args, ret)`` where
    ...
    >>> from pari_utils.parser import parse_prototype
    >>> proto = 'GD0,L,DGDGDG'
...
def gp_functions():
    """
    Iterator through GP function names

    >>> from pari_utils.parser import gp_functions
    >>> it = gp_functions()
    >>> [next(it) for _ in range(5)]
    ['Catalan', 'Col', 'Colrev', 'Euler', 'I']
    """
    pari_desc = read_pari_desc()
    for name in sorted(pari_desc):
        data = pari_desc[name]
        if data['class'] == 'basic' and '_' not in name and name != '%' and name != '%#':
            yield name
